# 7. 삼성전자 데이터에 KOSPI200 데이터를 엮어서 앙상블 모델을 DNN으로 구성하기
import numpy as np
import pandas as pd
...
x1_train = np.reshape(
    x1_train, (x1_train.shape[0], x1_train.shape[1] * x1_train.shape[2])
)
x1_test = np.reshape(x1_test, (x1_test.shape[0], x1_test.shape[1] * x1_test.shape[2]))
x2_train = np.reshape(
    x2_train, (x2_train.shape[0], x2_train.shape[1] * x2_train.shape[2])
)
...
print(x2_train_scaled[0, :])  # 결과치 확인하기
...
model.fit(
    [x1_train_scaled, x2_train_scaled],
    y1_train,
