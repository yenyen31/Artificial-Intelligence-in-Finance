...
# 데이터 파일을 저장할 data 폴더를 작업 폴더 하단에 만든 뒤, csv파일을 Panda형태로 저장
# header=0은 0번째 행을 헤더로 사용하겠다는 의미
df1 = pd.read_csv(
    ".\kospi200\data\kospi200.csv", index_col=0, header=0, encoding="cp949", sep=","
)
# 경로 주의!
print(df1)
print(df1.shape)

# 삼성전자의 주가 데이터를 동일한 방법으로 추가해서 가져오기
df2 = pd.read_csv(
    ".\kospi200\data\samsung.csv", index_col=0, header=0, encoding="cp949", sep=","
)
print(df2)
print(df2.shape)
